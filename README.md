...
1. ati_netft - A ROS2 wrapper for the ATI NetFT force torque sensor as a regular node by polling for the data. 
2. netft_hardware_interface - A ros2_control harware interface for the sensor that utilizes high-speed UDP streaming of the NetFT sensors.
...
### Option 2 (recommended): netft_hardware_interface package
...
> [!NOTE]  
> `cpt` and `cpf` values can be obtained from `<netft_ip>/netftapi2.xml` page
